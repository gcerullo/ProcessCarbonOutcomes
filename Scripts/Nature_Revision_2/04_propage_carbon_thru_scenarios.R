...
# single scenario set with all carbon, ACD, and discounted social cost outputs.
# =============================================================================
...
#add temporal tmiespan to all start landscape
all_start_landscape<- all_start_landscape %>%
...
scenarios2 <- readRDS("Inputs/MasterAllScenarios.rds")
scenario_composition <- rbindlist(scenarios2, use.names=TRUE) %>% 
  #proportion of TOTAL landscape [1000 parcels] in different habitat type 
  group_by(index, production_target) %>% 
  #total OG
  mutate(propOG = sum(num_parcels[habitat == "primary"])/1000)# get scenario composition
rm(scenarios2)
#----------------read in scenarios -------------------------------
...
filter_original_eq_habitat <- function(df_list) {
  df_list %>%
    map(~ .x %>%
          filter(!(original_habitat == habitat & harvest_delay != 0))
    )
}
...
#___________________________________________________________________
# B)2L - 2L modification. Forest that starts with twice-logged was re-harvested just before scenario start
# Takes a list of data frames and applies two operations to each:
...
scenarios <- scenarios %>%
  map(~ .x %>%
        mutate(harvest_delay = as.numeric(stringr::str_extract(harvest_delay, "\\d+")))
  )
...
for(i in seq_along(scenarios)) {
  ...
  add_carbon_fun <- function(x, draw_dt) {
    ...
    #also joins by functionalhabAge here (as these are not NA for non-primary hab)
  ...
  calculate_delays_per_transition_fun <- function(x) {
    ...
    #calculate parcels per delay transtion
  ...
  scenario_ACD_fun <- function(x) {
    ...
    #--------------------------------------------------------------
    # ACD is Mg / ha; 10 km² = 1000 ha
    x[, ACD_per_parcel := ACD * 1000]
    
    #--------------------------------------------------------------
    # STEP 2 — Correct staggered allocation of parcels
    #--------------------------------------------------------------
    # IMPORTANT:
    ...
    # Total carbon assigned to this transition-year:
    x[, ACD_10km2_stag := ACD_per_parcel * parcels_per_delay]
    
    #x <- x %>% scenario_row_order_visualisation_fun()
    
    
    
    
    #--------------------------------------------------------------
    # STEP 3 — Aggregate to habitat transition × year
    #--------------------------------------------------------------
    ...
    #--------------------------------------------------------------
    # STEP 4 — Aggregate to scenario × year
    #--------------------------------------------------------------
  ...
  #  # x[, ACD_10km2_stag := ACD_10km * (num_parcels)]
  #   
  #   
  ...
  carbon_yrs <- lapply(scenario_v, carbon_stock_years_fun)
  ...
  # #NOTE - the above will tend to overestimate uncertainty, because we are essentially summing uncertainty of each year
  # #We can instead report the average annual carbon in a given scenario, after having calculated annual uncertainty across draws 
  # # x: one draw of scenario trajectories
  # 
  # # Function to calculate mean annual carbon across draws, then mean across years with uncertainty
  #  # Step 0: Combine all posterior draws into one data.table
  # # Function to calculate mean annual carbon across draws, then mean across years with uncertainty
  # mean_carbon_60yr_fun <- function(scenario_draws_list, year_col = "true_year", acd_col = "scen_ACD_year") {
  #   
  #   # Step 0: Combine all posterior draws into one data.table
  #   ACD_years <- rbindlist(scenario_draws_list, use.names = TRUE, idcol = "draw")
  #   
  #   # Step 1: Mean across draws for each year (grouped by scenario)
  #   year_mean <- ACD_years[, .(
  #     mean_ACD_draws = mean(get(acd_col), na.rm = TRUE)
  #   ), by = c(year_col, "index", "production_target", "scenarioStart", "scenarioName")]
  #   
  #   # Step 2: Summarize across years to get 60-year mean and uncertainty
  #   mean_60yr_summary <- year_mean[, .(
  #     mean_60yr_ACD = mean(mean_ACD_draws, na.rm = TRUE),
  #     lwr_95 = quantile(mean_ACD_draws, 0.025, na.rm = TRUE),
  #     upr_95 = quantile(mean_ACD_draws, 0.975, na.rm = TRUE),
  #     lwr_80 = quantile(mean_ACD_draws, 0.10, na.rm = TRUE),
  #     upr_80 = quantile(mean_ACD_draws, 0.90, na.rm = TRUE)
  #   ), by = .(index, production_target, scenarioStart, scenarioName)]
  #   
  #   return(mean_60yr_summary)
  # }
  # 
  # # Apply function to your list of scenarios
  # carbon_yrs_summary <- mean_carbon_60yr_fun(scenario_i)
  
  # # Combine into single data.table, adding scenario index
  # carbon_yrs_summary <- rbindlist(carbon_yrs_summary)
  
  ##_________________________________________________________
  #quick plot to make sure we'eve properly propagated uncertainty:
  ##_________________________________________________________
  #Filter to just AllPrimary starting scenarios
  scenario_composition <- scenario_composition %>%  select(index, production_target,scenarioStart, scenarioName,propOG) %>%unique 
  
  scenario_composition %>% filter(index == "all_primary_CY_D.csv 481")
  carbon_df <- carbon_stock_years_summary %>%
  ...
  plot <- carbon_df %>%
    ...
    scale_color_gradient(
      name = "Proportion old growth",
      low = "#FFE8C2",   # light orange / peach
      high = "#CC5500"   # dark burnt orange
    )+
  ...
  # plot relative uncertainty = CI width / mean
  carbon_stock_years_summary[, rel_uncertainty :=
                               (upr_cum_stock_year_95 - lwr_cum_stock_year_95) / mean_cum_stock_year]
  
  # Plot relative uncertainty vs production targett to see if scenario uncertainty is changing a lot by production target
  carbon_stock_years_summary%>%
    ...
    labs(
      x = "Production target",
      y = "Relative uncertainty (95% CI width / mean)",
      title = "Posterior relative uncertainty across production targets"
    ) +
    theme_minimal(base_size = 13)
  
  #seems here likey higher producing scenarios delivering a lot of wood by selective logging can be much more uncertain
  ...
  scenario_v <- join_scenario_with_start_ACD(
    scenario_list = scenario_v,
  ...
  scenario_v<- lapply(scenario_v, flux_conversion_function)
  #x <- scenario_i[[1]]
  ...
  final_carbon2 <- lapply(scenario_v, final_carbon_fun, SDR = socialDR_2) %>% rbindlist(idcol = "draw")
  ...
  # store scenario-level output for later consolidation / export
  posterior_summary_all[[i]] <- posterior_summary_comb
...
saveRDS(
  ...
  file.path(step_rds_dir, "carbon_outcomes_across_500_draws.rds")
)
