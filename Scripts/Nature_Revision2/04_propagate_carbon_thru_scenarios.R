...
# Use a dedicated step/output label for the plantation-updated ACD draws so
# this run cannot overwrite earlier step-04 products.
RUN_LABEL <- "updated_plant"
STEP_ID <- paste0("04_propagate__", RUN_LABEL)
ACD_DRAWS_FILE <- "acdraws_aboveground_updated_plant.rds"

paths <- nr2_step_paths(STEP_ID)
...
load_acd_draws <- function(path = file.path(nr2_out_root, "02_draws", "rds", ACD_DRAWS_FILE)) {
  stopifnot(file.exists(path))
  readRDS(path)
}
...
#   uncertainty over long periods does not dominate interpretation.
{
  ...
  if (!exists("paths", inherits = TRUE) || !is.list(paths)) {
    if (!exists("STEP_ID", inherits = TRUE)) STEP_ID <- "04_propagate__updated_plant"
    paths <- nr2_step_paths(STEP_ID)
    nr2_ensure_dirs(paths)
  }
