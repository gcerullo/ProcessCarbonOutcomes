...
# - Write all outputs under nr2_out_root from _config.R (default: Desktop/carbon_data_plantation_models)
...
SSB <- SSB %>%
  mutate(
    # Assign wood density by planted species.
    wood_density = case_when(
      Species == "EP" ~ EP_wd,
      Species == "AF" ~ AB_wd
    ),
    ...
    # Convert per-tree biomass to per-hectare biomass using stand density.
    biomass_ha = tree_biomass * live_trees_ha,
    
    # Convert kg ha-1 to Mg ha-1.
    AGB = biomass_ha / 1000,
    
    # Convert aboveground biomass to aboveground carbon density.
    ACD = AGB * 0.47
  )
...
# ============================================================
  ...
  saveRDS(joint_model, file.path(nr_model_root, "unified_linear_carbon_model_updated_plant.rds"))

  # Posterior predictive checks by state
  states <- levels(all_data$state)
  pdf(file.path(nr_fig_dir, "posterior_pp_checks_by_state_updated_plant.pdf"), width = 8, height = 6)
  for (s in states) {
...
# ============================================================
  ...
  ggsave(
    filename = file.path(nr_fig_dir, "posterior_pred_trajectories_CI_updated_plant.png"),
    plot = p_ci,
    width = 12,
    height = 7,
    units = "in",
  ...
  ggsave(
    filename = file.path(nr_fig_dir, "posterior_pred_trajectories_spaghetti_updated_plant.png"),
    plot = p_spag,
    width = 12,
    height = 7,
    units = "in",
...
#  }
  ...
  saveRDS(draws_long, file.path(nr_rds_dir, "onemodel_ACD_draws_updated_plant.rds"))
  write.csv(draws_long, file.path(nr_rds_dir, "onemodel_ACD_draws_updated_plant.csv"), row.names = FALSE)
#}
